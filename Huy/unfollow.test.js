...
const { unfollowUser } = require('../controllers/profile.controller'); // Hàm unfollow cần thử nghiệm
...
jest.mock('../middlewares/limiter/limiter', () => ({
  followLimiter: (req, res, next) => next(), // Mock giới hạn tốc độ cho unfollow
...
describe('User Unfollow API - Real Database', () => {
  ...
  // Bộ thử nghiệm cho endpoint unfollow
  describe('PATCH /users/:id/unfollow', () => {
    it('should allow a user to unfollow another user', async () => {
      ...
      console.log(`Unfollow test passed: ${user1.email} đã unfollow ${user2.email}`);
    });

    it('should return 400 if relationship does not exist', async () => {
      const user1 = await createTestUser('user1', 'Test User One');
      const user2 = await createTestUser('user2', 'Test User Two');
      const token = generateToken(user1._id);
      ...
      expect(response.body.message).toBe('Relationship does not exist');
      console.log(`No relationship test passed: ${user1.email} không thể unfollow ${user2.email}`);
    });
    ...
    it('should return 500 if an error occurs', async () => {
      ...
      expect(response.body.message).toBe('Some error occurred while unfollowing the user');
      console.log(`Error test passed: Unfollow gây lỗi như mong đợi`);
    });
  });
});